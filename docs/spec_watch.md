...
`tbp` pins the upstream Design Tokens 2025.10 technical-report and schema endpoints it depends on in `spec-watch/targets.json` and `spec-watch/lock.json`.
...
2. Decide whether the upstream change affects tbp code, tests, schemas, docs, or release expectations.
3. Open or update a `bd` follow-up issue in the canonical repo root so the drift has an explicit owner.
4. If tbp should adopt the upstream change, update the code/docs as needed, run `python3 scripts/spec_watch.py refresh`, and commit the refreshed lock file in the same review.
