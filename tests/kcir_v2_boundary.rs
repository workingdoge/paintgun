...
fn kcir_v2_module_keeps_kernel_calls_in_compat() {
    let kcir_v2_src = read_from_manifest("crates/premath-kcir/src/kcir_v2.rs");
    assert!(
        !kcir_v2_src.contains("kcir_kernel"),
        "crates/premath-kcir/src/kcir_v2.rs should not reference kcir_kernel directly"
    );
    assert!(
        !kcir_v2_src.contains("kcir::"),
        "crates/premath-kcir/src/kcir_v2.rs should not call kernel items directly; route through compat"
    );
}

#[test]
fn kcir_v2_compat_module_hosts_kernel_bridge() {
    let compat_src = read_from_manifest("crates/premath-kcir/src/kcir_v2/compat.rs");
    assert!(
