...
pub const SWIFT_EMITTER_API_VERSION: &str = "tbp-swift-tokens/v1";
pub const KOTLIN_EMITTER_API_VERSION: &str = "tbp-kotlin-tokens/v1";
    ...
    fn color_expr(&self, v: &DtcgColor) -> Option<String> {
        ...
        if let Some(hex) = &v.hex {
            if let Some((r8, g8, b8)) = parse_hex_rgb(hex) {
                ...
                return Some(format!(
                    "tbpColor(red: {r}, green: {g}, blue: {b}, opacity: {a})"
                ));
            }
        }
    ...
    Lookup: FnMut(&Input) -> Vec<EmissionToken>,
{
    ...
    out.push_str("// Generated by tbp-rs\n");
    out.push_str(&format!(
        "// Native API version: {}\n",
    ...
    out.push_str("public typealias TBPColor = Color\n");
    out.push_str("@inline(__always) private func tbpColor(red: Double, green: Double, blue: Double, opacity: Double) -> TBPColor {\n");
    out.push_str("  Color(.sRGB, red: red, green: green, blue: blue, opacity: opacity)\n");
    out.push_str("}\n");
    out.push_str("#else\n");
    out.push_str("import Foundation\n");
    out.push_str("public struct TBPColor: Equatable {\n");
    ...
    out.push_str("}\n");
    out.push_str("@inline(__always) private func tbpColor(red: Double, green: Double, blue: Double, opacity: Double) -> TBPColor {\n");
    out.push_str("  TBPColor(red: red, green: green, blue: blue, opacity: opacity)\n");
    out.push_str("}\n");
    out.push_str("#endif\n\n");
    out.push_str("public enum TBPEmitterAPI {\n");
    out.push_str(&format!(
        "  public static let swiftVersion = \"{}\"\n",
        SWIFT_EMITTER_API_VERSION
    ));
    out.push_str("}\n\n");
    out.push_str("public enum TBPTokenValue {\n");
    out.push_str("  case color(TBPColor)\n");
    ...
    out.push_str("}\n\n");
    out.push_str("public struct TBPTokens {\n");
    out.push_str("  public static let values: [String: [String: TBPTokenValue]] = [\n");
    ...
    fn color_expr(&self, v: &DtcgColor) -> Option<String> {
        ...
        if let Some(hex) = &v.hex {
            if let Some((r8, g8, b8)) = parse_hex_rgb(hex) {
                ...
                return Some(format!("TBPColor(0x{:08X}u)", argb));
            }
        }
    ...
    Lookup: FnMut(&Input) -> Vec<EmissionToken>,
{
    ...
    out.push_str(&format!(
        "// Native API version: {}\n",
        KOTLIN_EMITTER_API_VERSION
    ));
    out.push_str("package tbp\n\n");
    out.push_str(&format!(
        "const val TBP_EMITTER_API_VERSION: String = \"{}\"\n\n",
        KOTLIN_EMITTER_API_VERSION
    ));
    out.push_str("data class TBPColor(val argb: UInt)\n\n");
    out.push_str("sealed class TokenValue {\n");
    out.push_str("  data class ColorVal(val value: TBPColor) : TokenValue()\n");
...
fn swift_package_manifest() -> String {
    ...
    out.push_str("  name: \"TBPTokens\",\n");
    out.push_str("  products: [\n");
    out.push_str("    .library(name: \"TBPTokens\", targets: [\"TBPTokens\"])\n");
    out.push_str("  ],\n");
    out.push_str("  targets: [\n");
    out.push_str("    .target(name: \"TBPTokens\"),\n");
    out.push_str("    .testTarget(name: \"TBPTokensTests\", dependencies: [\"TBPTokens\"])\n");
    out.push_str("  ]\n");
    out.push_str(")\n");
    out
}
...
fn swift_test_file() -> String {
    ...
    out.push_str("@testable import TBPTokens\n\n");
    out.push_str("final class TBPTokensTests: XCTestCase {\n");
    out.push_str("  func testValuesMapExists() {\n");
    out.push_str("    XCTAssertFalse(TBPTokens.values.isEmpty)\n");
    out.push_str("  }\n");
    out.push_str("}\n");
    out
}
...
pub fn emit_swift_package_scaffold(out_dir: &Path, swift_source: &str) -> std::io::Result<()> {
    ...
    write_utf8(
        &root.join("Sources/TBPTokens/TBPTokens.swift"),
        swift_source,
    )?;
    write_utf8(
        &root.join("Tests/TBPTokensTests/TBPTokensTests.swift"),
        &swift_test_file(),
    )?;
    Ok(())
}
...
fn kotlin_test_file() -> String {
    ...
    out.push_str("class TBPTokensSmokeTest {\n");
    out.push_str("  @Test\n");
    out.push_str("  fun valuesMapExists() {\n");
    out.push_str("    assertTrue(TBPTokens.values.isNotEmpty())\n");
    out.push_str("  }\n");
    out.push_str("}\n");
    out
}
...
pub fn emit_kotlin_module_scaffold(out_dir: &Path, kotlin_source: &str) -> std::io::Result<()> {
    let root = out_dir.join("kotlin");
    write_utf8(&root.join("settings.gradle.kts"), &kotlin_settings_gradle())?;
    write_utf8(&root.join("build.gradle.kts"), &kotlin_build_gradle())?;
    write_utf8(
        &root.join("src/main/kotlin/tbp/TBPTokens.kt"),
        kotlin_source,
    )?;
    write_utf8(
        &root.join("src/test/kotlin/tbp/TBPTokensSmokeTest.kt"),
        &kotlin_test_file(),
    )?;
    Ok(())
}
